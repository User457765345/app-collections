...
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget,  QPushButton, QTextEdit, QFileDialog, QLabel,QGridLayout,QFrame,QMessageBox
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 创建标签并设置固定大小和位置
        self.label1 = QLabel("翻译工具：", self)
        self.label1.setFixedWidth(200)
        self.label1.setFixedHeight(20)
        grid_layout.addWidget(self.label1,0,0)

        # 定义按钮和组件的位置和大小
        self.button_select_file = QPushButton("选择PDF文件", self)
        self.button_select_file.setFixedWidth(191)
        self.button_select_file.setFixedHeight(31)
        grid_layout.addWidget(self.button_select_file, 0, 1)

        self.help_button = QPushButton("帮助", self)
        self.help_button.setFixedWidth(60)  
        self.help_button.setFixedHeight(21)
        grid_layout.addWidget(self.help_button, 1, 0)  # 第二行第一列

        # 连接帮助按钮的点击信号到槽函数
        self.help_button.clicked.connect(self.show_help_message)

        self.button_select_folder = QPushButton("选择文件夹中的所有PDF文件", self)
        self.button_select_folder.setFixedWidth(191)
        self.button_select_folder.setFixedHeight(31)
        grid_layout.addWidget(self.button_select_folder, 1, 1)

        #分割线贯穿三列
        self.bottom_line = QFrame(container)
        self.bottom_line.setFrameShape(QFrame.HLine)      
        self.bottom_line.setGeometry(0, self.height() - 30, self.width(), 60)
        grid_layout.addWidget(self.bottom_line, 2, 0, 1, 3)

        # 创建标签并设置固定大小和位置
        self.label1 = QLabel("关键词提取工具：", self)
        self.label1.setFixedWidth(200)
        self.label1.setFixedHeight(20)
        grid_layout.addWidget(self.label1,3,0)

        self.help_button2 = QPushButton("帮助", self)
        self.help_button2.setFixedWidth(60)  
        self.help_button2.setFixedHeight(21)
        grid_layout.addWidget(self.help_button2, 4, 0)

        self.button_upload_config = QPushButton("上传配置文件", self)
        self.button_upload_config.setFixedWidth(191)
        self.button_upload_config.setFixedHeight(31)
        grid_layout.addWidget(self.button_upload_config, 3, 1)

        self.openButton = QPushButton('选择CSV文件', self)
        self.openButton.setFixedWidth(191)
        self.openButton.setFixedHeight(31)
        grid_layout.addWidget(self.openButton, 4, 1)

        self.extractButton = QPushButton('提取并保存数据', self)
        self.extractButton.setFixedWidth(191)
        self.extractButton.setFixedHeight(31)
        grid_layout.addWidget(self.extractButton, 5, 1)

        #分割线贯穿三列
        self.bottom_line = QFrame(container)
        self.bottom_line.setFrameShape(QFrame.HLine)      
        self.bottom_line.setGeometry(0, self.height() - 30, self.width(), 60)
        grid_layout.addWidget(self.bottom_line, 6, 0, 1, 3)

        self.label1 = QLabel("知识图谱工具：", self)
        self.label1.setFixedWidth(200)
        self.label1.setFixedHeight(20)
        grid_layout.addWidget(self.label1,7,0)

        self.help_button3 = QPushButton("帮助", self)
        self.help_button3.setFixedWidth(60)  
        self.help_button3.setFixedHeight(21)
        grid_layout.addWidget(self.help_button3, 8, 0)

        self.folder_select_button = QPushButton('选择文件夹', self)
        self.folder_select_button.setFixedWidth(191)
        self.folder_select_button.setFixedHeight(31)
        grid_layout.addWidget(self.folder_select_button, 7, 1)

        self.neo4j_button = QPushButton('在Neo4j生成知识图谱', self)
        self.neo4j_button.setFixedWidth(191)
        self.neo4j_button.setFixedHeight(31)
        grid_layout.addWidget(self.neo4j_button, 8, 1)

        

        

        # 将标签和文本框向右平移，假设按钮最宽为191，我们从第3列开始添加
        label_and_text_edit_column_start = 3  # 按钮宽度为191，假设按钮占据2个格子，所以从第3列开始

        # "尚未选择文件！"
        self.file_label = QLabel("尚未选择文件：")
        grid_layout.addWidget(self.file_label, 1, label_and_text_edit_column_start)

        # 第一个 QTextEdit
        self.text_edit = QTextEdit(self)
        self.text_edit.setReadOnly(True)
        self.text_edit.setFixedWidth(300)  # 可以设置一个固定宽度
        grid_layout.addWidget(self.text_edit, 2, label_and_text_edit_column_start)

        # "PDF提取已完成"
        self.status_label = QLabel("PDF提取已完成：")
        grid_layout.addWidget(self.status_label, 3, label_and_text_edit_column_start)

        # 第二个 QTextEdit
        self.csv_list = QTextEdit(self)
        self.csv_list.setReadOnly(True)
        self.csv_list.setFixedWidth(300)  # 可以设置一个固定宽度
        grid_layout.addWidget(self.csv_list, 4, label_and_text_edit_column_start)

        # 连接信号和槽
        self.button_select_file.clicked.connect(self.select_file)
        self.button_select_folder.clicked.connect(self.select_folder_for_pdf)
        ...
        self.folder_select_button.clicked.connect(self.select_folder_for_csv)
        self.neo4j_button.clicked.connect(self.convert_to_neo4j)

        #帮助信号
        self.help_button2.clicked.connect(self.show_help_message2)
        self.help_button3.clicked.connect(self.show_help_message3)

    def show_help_message(self):
        # 点击帮助按钮时显示的消息框
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Information)
        msg.setText("上传单个或多个pdf，使用kimi大模型进行分段和翻译，分段切片存储于文件夹，而翻译结果存储于同名txt。")
        msg.setWindowTitle("帮助-pdf工具")
        msg.setStandardButtons(QMessageBox.Ok)
        msg.exec()
    
    def show_help_message2(self):
        # 点击帮助按钮时显示的消息框
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Information)
        msg.setText("对于翻译结果进行处理。通过配置文件来搜索其同目录下的所有txt文件，从大模型获取去关键词。生成result+当前时间的新文件夹存储。“选择csv文件”按钮找到result.csv，读取信息。“提取并保存数据”来让它们被存储到相应名称的csv中，供后续工具检索。")
        msg.setWindowTitle("帮助-csv工具")
        msg.setStandardButtons(QMessageBox.Ok)
        msg.exec()

    def show_help_message3(self):
        # 点击帮助按钮时显示的消息框
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Information)
        msg.setText("选择第二部生成的result文件夹，确保其中包含所有需要的csv文件，生成对应的cypher语句和知识图谱。请确保neo4j已经在后台运行！")
        msg.setWindowTitle("帮助-csv工具")
        msg.setStandardButtons(QMessageBox.Ok)
        msg.exec()
